...
const commonModern = {
	...
	plugins: [new CleanWebpackPlugin()].concat(htmlPlugins('src/pages')),
}
...
const productionModern = {
	plugins: [new CopyPlugin({ patterns: ['src/google2689790980d4cc88.html'] }), new MiniCssExtractPlugin({ filename: path.join('./css', '[name].css') })],
	module: {
		rules: [
			{
				test: /\.scss$/,
